BEGIN;
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM public.audit_event_models WHERE action = 'path_invitation.canceled') THEN
    RAISE EXCEPTION 'cannot remove path invitation cancellation audit taxonomy while evidence exists';
  END IF;
END $$;
...
ALTER TABLE public.audit_event_models ADD CONSTRAINT audit_event_models_action_check CHECK (action IN (
  ...
  'path_invitation.accepted','path_invitation.rejected','path_ownership_transfer.created','path_ownership_transfer.listed',
  'path_ownership_transfer.accepted','path_ownership_transfer.declined','path_ownership_transfer.canceled'
));
COMMIT;
