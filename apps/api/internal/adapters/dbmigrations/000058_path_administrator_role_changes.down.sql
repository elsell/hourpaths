...
DELETE FROM public.notification_models
WHERE kind = 'path_member_role_changed' AND offered_role = 'administrator';
...
BEGIN
  ...
  SELECT path.owner_user_id INTO path_owner FROM public.path_models AS path
  WHERE path.id = path_identifier AND path.archived_at IS NULL FOR UPDATE;
  ...
  SELECT membership.role INTO target_role FROM public.path_membership_models AS membership
  WHERE membership.path_id = path_identifier AND membership.user_id = member_user_identifier
    AND membership.role = expected_member_role AND membership.role IN ('participant', 'supporter') FOR UPDATE;
  ...
  END IF;
  UPDATE public.path_membership_models SET role = requested_member_role
