...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.notification_models WHERE kind = 'path_member_left') THEN
    RAISE EXCEPTION 'cannot roll back path-member-leave notifications while rows remain';
  END IF;
END $$;
...
ALTER TABLE public.notification_models
  ...
  ADD CONSTRAINT notification_models_presentation_kind_check CHECK (
    ...
    OR (kind IN ('path_invitation_accepted', 'path_ownership_transfer_accepted', 'path_ownership_transfer_declined', 'path_ownership_transfer_canceled', 'path_deleted', 'new_follower', 'follow_request_accepted', 'practice_reaction', 'practice_comment', 'comment_heart') AND presentation_class = 'informational')
  ),
