...
export function BlockedAccountListView({
  ...
  >
    ...
    {state.nextCursor ? <Pressable
      accessibilityRole="button"
      accessibilityState={{ busy: state.loadingMore, disabled: state.loadingMore || state.busyUserId !== undefined }}
      disabled={state.loadingMore || state.busyUserId !== undefined}
      onPress={onLoadMore}
      style={styles.loadMore}
    >
      {state.loadingMore ? <ActivityIndicator color={mobileTheme.colors.accent} /> : null}
      <Text style={styles.actionLabel}>
        {i18n.t(state.loadingMore ? 'blocking.loadingMore' : 'blocking.loadMore')}
      </Text>
    </Pressable> : null}
  </ScrollView>;
}
...
const styles = StyleSheet.create({
  actionLabel: { color: mobileTheme.colors.accent, fontSize: 17, fontWeight: '600', lineHeight: 22 },
  ...
  stateAction: { alignItems: 'center', justifyContent: 'center', minHeight: mobileTheme.sizes.minimumTouchTarget },
