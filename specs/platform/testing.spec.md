...
CocoaPods is Bundler-locked, hosted iOS CI uses the macos-26 image's Homebrew
Ruby 3.4 with Bundler 2.6.9, verifies locked CocoaPods 1.16.2, Android CI selects an exact JDK patch, and signed
...
Android jobs must select Java `17.0.15+6`, install the exact SDK package
`cmake;3.22.1`, `ndk;27.1.12297006`, and `ndk;27.0.12077973` packages when the
hosted image does not already provide them, and fail closed unless the hosted
runner exposes Android platform `android-36`, build tools `36.0.0`, React
Native NDK `27.1.12297006`, AGP default NDK `27.0.12077973`, CMake `3.22.1`,
and Ninja. The hosted check must verify the
executable compiler/toolchain paths before Gradle runs. Ubuntu packages track
the hosted image's security-maintained state; downloaded non-Ubuntu tool
archives remain checksum-pinned.
...
runtime: `expo-auth-session 55.0.18`, `expo-web-browser 55.0.20`,
`expo-secure-store 55.0.18`, `expo-application 55.0.17`, `expo-crypto 55.0.19`,
and `expo-linking 55.0.17`. Their compensating checks are exact pins, resolved
