...
HourPaths trusts one configured OIDC authority at this boundary; Google and
Apple are upstream providers whose claims the authority normalizes into its own
signed tokens. The authority's exact issuer and opaque subject assigned to one
upstream identity form the identity key. That pair must remain stable across
HourPaths clients and email or display-name changes, while different upstream
identities remain distinct. A supplied email is trusted as account-reference
data only when the authority marks a non-empty value verified. An unverified or
empty email must not drive duplicate-email recovery or invitation consumption,
must not overwrite a previously stored verified provider email, and must be
stored as empty when first creating an identity. Verification provenance is
persisted with the provider email; pre-existing emails without provenance are
treated as unverified until a later verified provider claim refreshes them. An
absent optional Apple name must remain an empty editable onboarding seed, and
an Apple private-relay email must remain ordinary private account-reference data
without special matching or identity semantics.
...
The public session-exchange endpoint uses the generated API's ordinary stable
error codes: `unauthenticated` or `invalid_credential` for a rejected identity
credential, `forbidden` for an ineligible identity, `bad_request` for malformed
input, and `internal_error` for an unavailable dependency. Clients must use the
exchange context to present these codes as safe sign-in failures rather than as
an expired application session; HTTP status remains authoritative for any
credential-disposal decision.
...
Authorization outbox leases control claim ownership. A PostgreSQL-backed
per-resource serializer covers producer persistence through transaction commit
and is held by workers across each SpiceDB relationship write. PostgreSQL assigns
strictly increasing same-resource outbox timestamps across all history, including
completed rows, so producer clock skew or a database-clock regression cannot
reverse TOUCH and DELETE order or let a delayed worker resurrect access. Completion
...
authenticated affected resource owner can list only dead letters whose recorded
resource owner is that user, regardless of whether another user is the relationship
subject, through the same signed, stable cursor pagination used by other collections
and explicitly requeue one through the API. Requeue atomically clears
