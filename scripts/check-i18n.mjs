...
if (rootArgument !== -1 && !process.argv[rootArgument + 1]) {
  console.error('i18n check: --root requires a directory');
  process.exit(1);
}
...
function duplicateCatalogKeys(source) {
  const seen = new Set();
...
for (const file of localeFiles) {
  try {
    ...
    for (const key of duplicateCatalogKeys(source)) report(`${file} has duplicate key ${key}`);
    const catalog = JSON.parse(source);
    if (!isCatalog(catalog)) {
      report(`${file} catalog must be a JSON object`);
      continue;
    }
    if (Object.keys(catalog).length === 0) report(`${file} catalog must not be empty`);
...
for (const [file, catalog] of catalogs) {
  ...
  for (const key of keys) {
    if (key.endsWith('_one')) {
      ...
      if (!keys.includes(other)) report(`plural ${key} requires ${other}`);
    }
    if (key.endsWith('_other')) {
      const one = `${key.slice(0, -6)}_one`;
      if (!keys.includes(one)) report(`plural ${key} requires ${one}`);
    }
  }
}
